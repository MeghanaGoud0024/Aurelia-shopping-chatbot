...
- A **quota chip** in the chat header shows live provider quota and, next to it, a **Live / Offline switch**. Click the chip for the detail panel: per-minute token and request windows (genuinely live, from response headers), the daily ceiling (only knowable once Groq states it in a 429, and timestamped as of when that happened rather than implied to be current), and a rough session token counter. The switch forces every turn through the deterministic fallback planner regardless of whether the API key works - same tools, same audit trail, only the reply's wording changes - which is the fast way out of exactly the free-tier daily-quota wall this project hit repeatedly while building it. See [`app/agent/llm.py`](app/agent/llm.py)'s `QuotaTracker` for what is and isn't a live number.
...
| `GET` | `/api/ops/llm-status` | Live provider quota and current mode (live vs. forced fallback) |
| `POST` | `/api/ops/llm-mode` | Toggle forced fallback on or off, `{"forced_fallback": true\|false}` |
...
**`Rate limit reached ... tokens per minute`** - free provider tiers are tight. The provider's own wait hint and backs off as it's has a limit. If it persists, raise the tier, or reduce prompt size further by leaving `AURELIA_TOOL_ROUTING_ENABLED=true`.
